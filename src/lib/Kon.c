...
int vozvrat_kod=kod_sbros;
	...
	if (deystvie==0)
	{
		...
		if (vozvrat_kod!=0)
		{
			if (doska[y_start][x_start]!='N' && doska[y_start][x_start]!='n')
			{
		...
		if (vozvrat_kod!=0)
		{	
			if (((y_start!=y_end+1) || (x_start!=x_end+2)) && ((y_start!=y_end+2) || (x_start!=x_end+1)) && ((y_start!=y_end+2) || (x_start!=x_end-1)) && ((y_start!=y_end+1) || (x_start!=x_end-2)) && ((y_start!=y_end-1) || (x_start!=x_end-2)) && ((y_start!=y_end-2) || (x_start!=x_end-1)) && ((y_start!=y_end-2) || (x_start!=x_end+1)) && ((y_start!=y_end-1) || (x_start!=x_end+2)))
			{
				printf("Erorr: Kon tak ne hodit");
				vozvrat_kod=0;
				return vozvrat_kod;	
			}
		}
